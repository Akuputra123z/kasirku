...
class MarketplaceController extends Controller
{
    ...
    public function allProducts(Request $request)
    {
        ...
        $products = Product::visibleOnline()
            ...
            ->when($search, fn ($q, $s) => $q->where('name', 'like', "%{$s}%"))
            ->when($categorySlug, $this->categoryFilterCallback($categorySlug))
            ->when($sort === 'price_low', fn ($q) => $q->orderByRaw('COALESCE(online_price, price) ASC'))
            ->when($sort === 'price_high', fn ($q) => $q->orderByRaw('COALESCE(online_price, price) DESC'))
            ->when(in_array($sort, ['latest', 'newest']), fn ($q) => $q->latest())
            ->when($priceMin, fn ($q, $v) => $q->whereRaw('COALESCE(online_price, price) >= CAST(? AS NUMERIC)', [$v]))
            ->when($priceMax, fn ($q, $v) => $q->whereRaw('COALESCE(online_price, price) <= CAST(? AS NUMERIC)', [$v]))
